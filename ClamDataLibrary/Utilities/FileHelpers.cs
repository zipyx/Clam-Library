        ...
        private static readonly Dictionary<string, List<byte[]>> _fileSignature = new Dictionary<string, List<byte[]>>
                {
                    ...
                    { ".pdf", new List<byte[]> 
                        { 
                            ...
                            new byte[] { 0x25, 0x50, 0x44, 0x46, 0x2D, 0x31, 0x2E, 0x36},
                            new byte[] { 0x25, 0x50, 0x44, 0x46, 0x2D, 0x31, 0x2E, 0x34}
                        } 
                    },

                    // Windows Program --> DOS Executable file
                    { ".exe", new List<byte[]> { new byte[] { 0x4D, 0x5A } } },


                    // Video Format File Signatures
                    { ".mkv", new List<byte[]>
                        {
                            new byte[] { 0x1A, 0x45, 0xDF, 0xA3, 0x93, 0x42, 0x82, 0x88 },
                            new byte[] { 0x1A, 0x45, 0xDF, 0xA3, 0xA3, 0x42, 0x86, 0x81 },
                    ...
                    { ".3gp5", new List<byte[]> { new byte[] { 0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70 } } }, // --> MP4 Apparent
                    ...
                    // Music Format File Signatures
                    { ".mp3", new List<byte[]>
                        {
                            new byte[] { 0x49, 0x44, 0x33 },
                            new byte[] { 0x49, 0x44, 0x33, 0x04},
                            new byte[] { 0x49, 0x44, 0x33, 0x03},
                            new byte[] { 0x49, 0x44, 0x33, 0x2E}
                        }
                    },
                    { ".wav", new List<byte[]> { new byte[] { 0x52, 0x49, 0x46, 0x46 } } },
                    { ".ogg", new List<byte[]> { new byte[] { 0x4F, 0x67, 0x67, 0x53, 0x00, 0x02, 0x00, 0x00} } },

                    // Audio Book
                    { ".aax", new List<byte[]> { new byte[] { 0x00, 0x00, 0x00, 0x24, 0x66, 0x74, 0x79, 0x70 } } },
                    ...
                    // Document File Signature Extensions
                    { ".doc", new List<byte[]> 
                        { 
                        new byte[] { 0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0xA1, 0xE1 },
                    ...
                    { ".txt", new List<byte[]> 
                        {
                            new byte[] { 0x64, 0x6F, 0x63, 0x0D, 0x64, 0x6F, 0x63, 0x78 },
                            new byte[] { 0x64, 0x6F, 0x63, 0x0D, 0x64, 0x6F, 0x63, 0x78, 0x0D, 0x0A },
                        } 
                    },
                    { ".wpd", new List<byte[]> 
                        {
                            new byte[] { 0xFF, 0x57, 0x50, 0x43 }
                        } 
                    },
